...
class MultiAnchorTests(unittest.TestCase):
    ...
    def test_01_normalizes_flat_selection(self):
        anchors = normalize_anchors([
            {"frame_time_sec": 12.5, "frame_key": "f1.jpg", "x": .1, "y": .2, "w": .1, "h": .2}
        ])
    ...
    def test_02_normalizes_nested_bbox(self):
        anchors = normalize_anchors([
            {"time_sec": 8, "bbox": {"x": .2, "y": .3, "w": .2, "h": .3}}
        ])
        self.assertEqual(anchors[0]["x"], .2)

    def test_03_discards_invalid_items(self):
        anchors = normalize_anchors([
            None,
            {"time_sec": -1, "x": .1, "y": .1, "w": .1, "h": .1},
            {"time_sec": 2, "x": .95, "y": .1, "w": .2, "h": .1},
        ])
        self.assertEqual(anchors, [])

    def test_04_deduplicates(self):
        item = {"time_sec": 2, "x": .1, "y": .1, "w": .1, "h": .1}
        self.assertEqual(len(normalize_anchors([item, dict(item)])), 1)

    def test_05_sorts_and_caps(self):
        items = [
            {"time_sec": t, "x": .1, "y": .1, "w": .1, "h": .1}
            for t in [9, 2, 7, 3, 8, 1]
        ]
        self.assertEqual([item["t"] for item in normalize_anchors(items)], [1, 2, 3, 7, 8])

    def test_06_filters_anchors_for_window(self):
        anchors = normalize_anchors([
            {"time_sec": 10, "x": .1, "y": .1, "w": .1, "h": .1},
            {"time_sec": 50, "x": .2, "y": .2, "w": .1, "h": .1},
        ])
        self.assertEqual([a["t"] for a in anchors_for_window(anchors, 0, 45)], [10])

    def test_07_anchor_selects_correct_track(self):
        anchors = normalize_anchors([
            {"time_sec": 40, "frame_key": "f40", "x": .45, "y": .2, "w": .10, "h": .25}
        ])
        track_map = {
            1: [{"t": 5, "bbox": {"x": .10, "y": .2, "w": .10, "h": .25}, "conf": .9}],
            7: [{"t": 5, "bbox": {"x": .45, "y": .2, "w": .10, "h": .25}, "conf": .9}],
        }
    ...
    def test_08_anchor_overrides_continuity(self):
        anchors = normalize_anchors([
            {"time_sec": 40, "x": .70, "y": .2, "w": .10, "h": .25}
        ])
        track_map = {
            1: [{"t": 0, "bbox": {"x": .10, "y": .2, "w": .10, "h": .25}},
                {"t": 5, "bbox": {"x": .12, "y": .2, "w": .10, "h": .25}}],
            2: [{"t": 5, "bbox": {"x": .70, "y": .2, "w": .10, "h": .25}}],
        }
        match = select_window_track(
            ...
            window_start=35,
            window_end=80,
            previous_bbox={"x": .10, "y": .2, "w": .10, "h": .25},
        )
    ...
    def test_09_continuity_fallback(self):
        match = select_window_track(
            {4: [{"t": 0, "bbox": {"x": .21, "y": .20, "w": .10, "h": .25}}]},
            [],
            window_start=45,
            window_end=90,
            previous_bbox={"x": .20, "y": .20, "w": .10, "h": .25},
        )
    ...
    def test_10_rejects_bad_continuity(self):
        match = select_window_track(
            {4: [{"t": 0, "bbox": {"x": .80, "y": .70, "w": .03, "h": .04}}]},
            [],
            window_start=45,
            window_end=90,
            previous_bbox={"x": .20, "y": .20, "w": .10, "h": .25},
    ...
    def test_12_window_spans_all_anchors(self):
        self.assertEqual(compute_tracking_window([100, 300], 500, 60, 60), (40.0, 320.0))
    ...
    def test_14_preserves_timestamp_zero(self):
        anchors = normalize_anchors([
            {"frame_time_sec": 0, "x": .1, "y": .1, "w": .1, "h": .2}
        ])
    ...
    def test_16_assigns_each_anchor_to_one_overlapping_window(self):
        anchors = normalize_anchors([
            {"frame_time_sec": 0, "x": .1, "y": .1, "w": .1, "h": .2},
            {"frame_time_sec": 42, "x": .2, "y": .1, "w": .1, "h": .2},
            {"frame_time_sec": 78, "x": .3, "y": .1, "w": .1, "h": .2},
        ])
    ...
    def test_17_stitches_distinct_local_ids_within_one_window(self):
        ...
        self.assertEqual([bbox["x"] for bbox in bboxes], [.20, .70])
